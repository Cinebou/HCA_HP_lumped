...
class balance(MOF):
    ...
    def en_balance_gas(self):
        ...
        dTgasdt = (en_flow + self.Htrans - mRT) / (self.m_gas*cp_gas)

        # Loger
        log_output.log_h_gas_msg(dTgasdt, en_flow, self.Htrans, mRT)
        return dTgasdt
    ...
    def en_balance_sor(self):
        ...
        dTsordt = (self.Htrans + Hads + pass_HTF) / self.cp_sor
        return dTsordt


    # sumarize the time differential equations into ODE system
    def equations(self, var, t):
        ...
        # recoed the value for next cycle
        self.m_gas_previous = self.m_gas

        # write down the log
        log_output.log_mass_msg( self.m_in, self.m_ads, self.m_out)
        
        print('current time is  : ', t)
        return [dmsordt, dTgasdt, dTsordt]
    ...
    def solver(self):
        ...
        var0 = [self.loading_init, self.gas_T_init, self.mof_T_init]
        # solver
        sol = odeint(self.equations, var0, self.t)
        
        # answer
        self.loading_list = sol[:,0]
        self.gas_T_list   = sol[:,1]
        self.mof_T_list   = sol[:,2]
        ...
        # plot graph
        plot_data = [self.loading_list,  self.gas_T_list, self.mof_T_list,T_HTF_out_list]
        legends = ['loading','temperature of gas','temperature of sorbent','temperature of HTF out']
